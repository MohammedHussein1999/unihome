...
export default function SubmitAnswer() {
  ...
  const fetchQuizzes = async () => {
    try {
      ...
      console.log("Fetched quizzes:", sortedQuizzes);
    } catch (error) {
                ...
                <div className="w-full max-w-4xl bg-white rounded-3xl shadow-md p-6 mb-5">
                  ...
                  <div className="space-y-2">
                    {question.answers.map((choice, choiceIndex) => (
                      <div
                        key={choice.id}
                        className={`border-2 border-gray-300 rounded-3xl flex justify-between items-center cursor-pointer transition-all duration-300 ${
